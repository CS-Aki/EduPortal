...
if(isset($_GET["user"])){
        ...
        $examGrades = $listController->getExamGrades($_GET["post"], $_GET["user"]);
        $quizGrades = $listController->getQuizGrades($_GET["post"], $_GET["user"]);
...
if(!isset($_GET["user"])){
    if($postDetails[0]["content_type"] == "Exam"){
        ...
        $examGrades = $listController->getExamGrades($_GET["class"], $_GET["post"]);
        
        $yourScore = array();

        $currentAttempt = 0;
        $quizContent = $instrCtrlr->getExamContent($postId, $classCode);
        $attempt = $quizContent[0]["attempt"];
        $totalScore = 0;
        $totalCorrectAnsCount = array();
        $j = 0;
