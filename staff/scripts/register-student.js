$(document).ready(function() {
    ...
    $("#signUpForm").submit(function(event) {
        ...
        let formData = {
            ...
            $.ajax({
                url: "includes/register-student.php",
                type: "POST",
                data: formData,
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                newRows.each(function() {
                                    ...
                                    var rowData = [];
                                    rowData.push(row.find('.student_id').html());  // Instructor Code (First column)
                                    rowData.push(row.find('.student_id_text').html());  // Instructor Code (First column)
                                    rowData.push(row.find('.student_name').html());     // Instructor Name
                                    rowData.push(row.find('.student_email').html());    // Email
                                    rowData.push(row.find('.student_status').html());   // Status
                                    rowData.push(row.find('.view_student_profile').parent().html()); // Edit action
